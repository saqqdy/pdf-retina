...
export default defineComponent({
	...
	setup(props, { slots, emit }) {
		...
		const canvasList = computed(() => {
			...
			if (pdfView.value && pdfView.value.numPages) {
				for (let i = 1; i <= pdfView.value.numPages; i++) {
					list.push({
						id: 'canvas_' + i,
						pageNumber: i
					})
		...
		return () =>
			h(
				...
				[
					...
					canvasList.value.map(async ({ id, pageNumber }, index) => {
						if (!pdfView.value) return
						...
						// 	// 渲染完毕后的业务操作
						// })
						// if (pageNumber === canvasList.value.length) {
						// 	proxy.$loading.hide()
						// }

						return h('canvas', { ref: canvasRef, key: id, height, width, style })
					})
				]
			)
		// return () => (
